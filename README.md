...
| 3 | [**gyro sup**](projects/gyro-sup.md) | Gyro, six-axis logging, Gyroflow workflow | **released** — [fpGyroSup v1.3](https://github.com/ijigen/fpSup/raw/main/gyro/release/fp-gyro-sup-v1.3.zip) · [notes](gyro/release/), GCSV and JSON written during the take, portrait too, no computer (CinemaDNG) |
...
- **The camera writes its own Gyroflow `.gcsv` and lens profile while it
  records.** fpGyroSup v1.3 streams the GCSV during the take and writes the JSON
...
| 3 | [**gyro sup**](projects/gyro-sup.md) | Gyro、六軸記錄與 Gyroflow 工作流 | **已發布** — [fpGyroSup v1.3 下載](https://github.com/ijigen/fpSup/raw/main/gyro/release/fp-gyro-sup-v1.3.zip) · [說明](gyro/release/),錄影中就寫好 GCSV 與 JSON,直拿也行,全程不接電腦(CinemaDNG) |
...
- **IMX410 的 ISO 與增益鏈完整反編譯**,並做成互動說明頁,明確區分「韌體已確認」
  與「仍需實機量測的 OTP 值」。
- **相機在錄影中就自己寫出 Gyroflow 的 `.gcsv` 與鏡頭 profile**。fpGyroSup v1.3
