...
|---|---|---|
<!-- releases:begin:en -->
| [`fpsup-gyro`](releases/fpsup-gyro-v1.13/) | v1.13 | Writes Gyroflow's `.gcsv` and `.json` while recording — every sample at 2500 Hz, distortion read off the lens. The profile now carries the size the take was actually recorded at, which open gate got wrong. |
...
|---|---|---|
<!-- releases:begin:zh -->
| [`fpsup-gyro`](releases/fpsup-gyro-v1.13/) | v1.13 | 錄影當下就把 Gyroflow 要的 `.gcsv` 與 `.json` 寫進片段資料夾。2500 Hz 每個樣本都在,畸變直接讀鏡頭。profile 的尺寸改成實際錄下的那個 —— open gate 下先前是錯的。 |
