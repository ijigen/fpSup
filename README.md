...
| 4 | [**open gate**](projects/open-gate.md) | Recording the sensor's full 3:2 area | **v0.2.1a alpha** — native UI plus the four-callsite shutter-angle and loader I-cache fixes; 29.97p/180° received a provisional cold-boot visual pass, exact readback and other regressions remain |
...
- **Open gate works at every ISO, with native Settings and Quick Set UI.**
  ...
  take, not inferred. v0.2.1a also corrects the shutter-angle path at four local
  callsites and synchronizes the loader's instruction cache. A battery-out
  29.97p/180° idle-view comparison received a provisional visual pass; exact
  readback, other frame rates, sustained-media and playback-with-UI regressions
  remain.
...
| [`fpsup-og3k`](releases/fpsup-og3k-v0.2.1a/) | v0.2.1a | 感光元件完整的 3:2 面積 —— 3024×2010 CinemaDNG、DNG 裁切 3008×2000,八個幀率從 23.976 到 100。**Settings 與 QS 已有原生 OG3K UI,包括三選項底欄。**mode 98 / 117 是感光元件原生 2×2 binning 的 3:2 讀出,ISP 不做縮放;全 ISO 正確。v0.2.1a 將快門角度的四個本地 callsite 全部導向有嚴格守衛的 nominal FPS 修正,並讓 loader 在放置補丁後清除指令快取。完整斷電冷開機後,29.97p/180° 的真實 FHD/OG3K idle 曝光取得暫定目視通過;精確讀值、其他幀率、長時間錄影、新 UI runtime 回放與非啟用 UI variants 尚未驗證,因此仍是 alpha。 |
...
| 4 | [**open gate**](projects/open-gate.md) | 錄下感光元件完整的 3:2 面積 | **v0.2.1a alpha** —— 原生 UI、四路快門角度與 loader I-cache 修正已整合;29.97p/180° 取得暫定冷開機目視通過,精確讀值與其他回歸仍待測 |
...
- **Open gate 可用,全 ISO,原生 UI 已整合。** 3:2 讀出的 3024×2010,整幅邊到邊 —— 解檔實錄驗證,不是推論。v0.2.1a 另修正快門角度的四個本地 callsite 與 loader 指令快取同步;完整斷電冷開機後,29.97p/180° idle 曝光取得暫定目視通過。精確讀值、其他幀率、長時間寫入與新 UI runtime 回放仍待回歸。
