...
|---|---|---|
| [**fpGyroSup v1.11a**](https://github.com/ijigen/fpSup/raw/main/gyro/release/fp-gyro-sup-v1.11a.zip) | `.gcsv` and `.json`, during the take, every sample | nothing to do |
...
| | 相機寫出 | 轉檔 |
|---|---|---|
| [**fpGyroSup v1.11a**](https://github.com/ijigen/fpSup/raw/main/gyro/release/fp-gyro-sup-v1.11a.zip) | 錄影當下寫 `.gcsv` 與 `.json`,一筆不漏 | 不用做 |
...
- **MOV:** no sidecars in v1.11a (it records through a path this build does
  not hook).
...
- **MOV：** v1.11a 沒有 sidecar（MOV 走的是這一版沒有掛鉤的另一條錄影路徑）。v1.1 仍會替
  MOV 寫 `.GYR`。
- **外接 SSD、UHD、變焦鏡：** 尚未測試。
