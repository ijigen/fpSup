...
**Status: working. 3032×2012 3:2 CinemaDNG at 29.97, whole frame — test build, not a release.**

用感光元件完整的 3:2 面積錄影,而不是相機裁出來的 16:9 視窗。
**狀態:可用。3032×2012 3:2 CinemaDNG @29.97,整張畫面 —— 測試版,不是正式 release。**
...
```
3032 × 2012 · 3:2 · 12-bit · 29.97003 fps · rolling shutter 9.221 ms
9,229,824 bytes per frame · 274.2 MB/s
```
...
|---|---|---|---|
| **open gate** | **117** | **3032×2012 2×2 3:2** | **9.221 ms** |
...
- No menu of its own: it rides on **CINE / CinemaDNG / 12-bit / FHD / 29.97p**.
  Reselect that combination after boot so the picker and profile rebuild.
- Live view flickers.
- In-camera playback does not display the material; check the DNGs on a computer.
...
```
3032 × 2012 · 3:2 · 12-bit · 29.97003 fps · 捲簾 9.221 ms
每幀 9,229,824 bytes · 274.2 MB/s
```

從卡上量的,不是推的:完成的 clip 四邊相鄰列相關性是 +0.876 ~ +0.933。
先前 producer 只填左上角的那些 clip,右邊是 +0.048、下面是 +0.053。整張都是畫面。

**274.2 MB/s 比相機每天在寫的 UHD 29.97(mode 7,376.2 MB/s)低 27%**,
...
- **只適用韌體 Ver.5.02。** AutoRun 語言沒有執行期版本 guard。
- 沒有自己的選單,寄生在 **CINE / CinemaDNG / 12-bit / FHD / 29.97p**。
  開機後要重新選一次那組設定,讓 picker 與 profile 重建。
- 即時取景會閃爍。
- 機身回放無法正常顯示,請用電腦看 DNG。
- Hook 的 payload 佔 `0xC072F800–0xC072F8E8`、telemetry 佔 `0xC072FA00–0xC072FA0F`,
  與主機端工具的暫存區重疊。武裝期間可以用單純的 `mem get`／`mem set`,
  但**不能**用 `getfile.py`、`putfile.py`、`inject.py`、`callfn.py`。
