#!/usr/bin/env python3
"""Build the development Focus Lift card through fpSup's shared loader.

Derived from the research devkit's RAW View builder; the FOCUS_LIFT assembly
...
def write_csv_inc(focus_lift=False):
    ...
    (HERE / f'{prefix}_fv.inc').write_text(f'/* 由 build_focus_lift.py 產生,勿手改 */\n.equ NFV, {len(files)}\n.equ NGL, {len(glyphs)}\n')
    out = ['/* 由 build_focus_lift.py 產生,勿手改 */', '.align 2', 'fvtab:']
...
def hook_sites(focus_lift=False):
    ...
    if focus_lift:
        ...
        sites += [(0xC02D2E50, 'Focus Lift preview tone pointer')]
...
def write_page_inc():
    ...
    out = ['/* 由 build_focus_lift.py 產生,勿手改 */', '.align 2', 'page_tab:']
...
def write_mat_inc():
    ...
    out = ['/* 由 build_focus_lift.py 產生,勿手改 */', '.align 2',
...
def write_sat_inc():
    ...
    out = ['/* 由 build_focus_lift.py 產生,勿手改 */', 'sat_tab:']
...
def write_dngcm_inc():
    ...
    b = rv_dngcm.blob()
    out = ['/* 由 build_focus_lift.py 產生,勿手改 */', 'cm_buf:        /* 初值 = 第 0 級(原廠 OFF),stub_gs 改寫 */']
...
def build_blob(focus_lift=True, focus_lift_default=None):
    if not focus_lift:
        raise ValueError("This builder packages only the Focus Lift variant")
    if focus_lift_default is not None and (not focus_lift or focus_lift_default not in (1, 2, 3)):
        raise ValueError('Startup level requires Focus Lift and a level from 1 to 3')
    ...
    variant = ['FOCUS_LIFT=1'] if focus_lift else []
    ...
    if focus_lift:
        assert 0x3200 + len(res) <= 0x5800, 'Focus Lift resident overlaps matrix storage'
...
def main():
    ...
    ap.add_argument('--default-level', type=int, choices=(1, 2, 3), default=2,
                    help='Startup Focus Lift level (default: +2)')
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        bf = tmp / 'focus-lift.bin'
        bf.write_bytes(blob)
        cmd = [sys.executable, str(SHELL / 'build_autorun.py'), '--loader',
               '--banner', 'fpSup-DEV-FOCUS-LIFT!', '--boot-bin', f'{bf}:0',
    ...
    (a.out / 'focus-lift.bin').write_bytes(blob)
    print(f'Focus Lift: {len(blob)} bytes; default +{a.default_level}; {info}')
    return 0
